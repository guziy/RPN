...
def get_monthly_accumulations_area_avg(data_dir="/HOME/huziy/skynet3_rech1/Netbeans Projects/Python/RPN/lake_effect_analysis_Obs_monthly_1980-2009",
    ...
    logger.debug(f"Reading data from {data_dir}")
    ...
    for month in months_of_interest:
        ...
        if len(files) == 0:
            search_pattern = f"{data_dir}/*m{month}-{month}{fname_suffix}"
            logger.debug(f"trying to search for {search_pattern}")
...
def main(varname="snow_fall"):
    ...
    label_to_datapath = OrderedDict([
        ("Obs", data_root / "lake_effect_analysis_daily_Obs_monthly_icefix_1980-2009"),
    ...
    for label, datapath in label_to_datapath.items():
        series = get_monthly_accumulations_area_avg(data_dir=str(datapath), varname=varname)
    ...
    # ax.grid()
    img_file = "hles_histo_all_m{}_percentages.png".format("_".join([str(m) for m in selected_months]))
    print(f"Saving plot to {img_file}")
    fig.savefig(img_dir / img_file, bbox_inches="tight", dpi=400, transparent=True)
