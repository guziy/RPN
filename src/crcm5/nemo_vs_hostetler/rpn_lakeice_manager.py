...
class RPNLakeIceManager(object):
    ...
    def __init__(self, samples_dir="", start_year=-np.Inf, end_year=np.Inf):
        ...
        self.year_month_date_to_file_list = get_monthyeardate_to_paths_map(file_prefix="pm", start_year=start_year, end_year=end_year, samples_dir_path=self.samples_dir)
    ...
    def get_lake_fraction_for_date(self, the_date=None):
        ...
        if the_date not in self.cached_data:
            ...
            self.cached_data = mr.get_all_time_records_for_name_and_level(varname=self.varname, level=self.level, level_kind=self.level_kind)
            mr.close()
